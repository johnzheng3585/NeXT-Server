...
install() {
    bash <(curl -Ls https://github.com/johnzheng3585/NeXT-Server/raw/main/release/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://github.com/johnzheng3585/NeXT-Server/raw/main/release/install.sh) $version
...
update_shell() {
    wget -q -O /usr/bin/uim-server https://github.com/johnzheng3585/NeXT-Server/raw/main/release/uim-server.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_menu() {
    echo -e "
--- https://github.com/johnzheng3585/NeXT-Server ---
  0. 修改配置
————————————————
  1. 安装 UIM-Server
  2. 更新 UIM-Server
